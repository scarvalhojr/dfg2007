...
\paragraph{Microarray design / Primer design.} 
...
information usually need to be represented twice.
...
\item[(A)] Investigation of the proposed Dual Sparse De-Bruijn Subgraphs
  (DSDS).
  
\item[(B)] Implement of a genome assembler, so that we can compare the
  ...
  \item Although genome assembling may look like an old fashion
    subject, the appearance of complete new sequencing methods in the
...
\item[(C)] Adaptation the repeat detection method being developed in our
  group to use DSDS.
  
\item[(D)] The applications validation is a natural final step.  For
...
Sparse De-Bruijn Subgraphs (DSDS) and designing the data structures for
implementing the genome assembler (phase B) and integrating it with our existing
...
Among the functions the DSDS data structure may offer, we point out:
...
In this phase a post-doctorand researcher will lead the design of the necessary
...
implementation is likely to require changes in the design of the data structue.
...
lead by a post-doctorand researcher. An initial working implementation providing
...
post-doctorand researchers (with the other programmer working independently on
...
single programmer under the supervision of both post-doctorand researchers, and
...
phase in concluded, both students will be free to concentrate in the
...
of the results will the responsability of both post-doctorand researchers, with
the help of both students and biologists from the Genome Research group of Prof.
\ Bernd Weisshaar and from the group of Dr. Felipe Rodrigues da Silva at
